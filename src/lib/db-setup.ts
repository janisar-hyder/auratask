...
export const setupDatabase = async () => {
  ...
  const { error: uuidError } = await supabase.from('_sql').select('*').execute(`
  ...
  // Create tasks table
  const { error: tasksError } = await supabase.from('_sql').select('*').execute(`
  ...
  const { error: statsError } = await supabase.from('_sql').select('*').execute(`
